			...
			if(newCell.isOcupied()) {	// new cell is ocupied
				if(!newCell.getPlayer().isActive()) {}
					...
					if(getPlayer() instanceof AutomaticPlayer) {
						try {
							ThreadAwakener a = new ThreadAwakener();
							a.start();
							System.out.println("Inicio do bloqueamento do jogador " + getPlayer().getIdentification());
							a.wait();
							System.out.println("Fim do bloqueamento do jogador " + getPlayer().getIdentification());
